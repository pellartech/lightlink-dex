...
class TokenSafetyLookupTable {
  ...
  update(lists: ListsState) {
    ...
    // Mark all LightLink COMMON_BASES tokens as safe (they aren't in the Uniswap default list)
    COMMON_BASES[LIGHTLINK_CHAIN_ID]?.forEach((base) => {
      this.dict[base.wrapped.address.toLowerCase()] = TOKEN_LIST_TYPES.UNI_DEFAULT
    })
